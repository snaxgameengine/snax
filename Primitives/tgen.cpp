...
namespace tgen
{
	...
	void computeCornerTSpace(const std::vector<VIndexT>& triIndicesPos,
		...
		const std::vector<Vec3>& positions3D,
		const std::vector<Vec2>& uvs2D,
		std::vector<Vec3>& cTangents3D,
		std::vector<Vec3>& cBitangents3D)
	{
		const std::size_t numCorners = triIndicesPos.size();

		cTangents3D.resize(numCorners);
		cBitangents3D.resize(numCorners);
		...
		for (std::size_t i = 0; i < triIndicesPos.size(); i += 3)
		{
			...
			for (std::size_t j = 0; j < 3; ++j)
			{
				...
				subVec3((const RealT *)&positions3D[v1PosIdx],
					(const RealT*)&positions3D[v0PosIdx],
					edge3D[j]);

				subVec2((const RealT*)&uvs2D[v1UVIdx],
					(const RealT*)&uvs2D[v0UVIdx],
					edgeUV[j]);
			}
			...
			for (std::size_t j = 0; j < 3; ++j)
			{
				...
				RealT* resultTangent = (RealT*)&cTangents3D[(i + j)];
				RealT* resultBitangent = (RealT*)&cBitangents3D[(i + j)];
	...
	void computeVertexTSpace(const std::vector<VIndexT>& triIndicesUV,
		const std::vector<Vec3>& cTangents3D,
		const std::vector<Vec3>& cBitangents3D,
		std::size_t                  numUVVertices,
		std::vector<Vec3>& vTangents3D,
		std::vector<Vec3>& vBitangents3D)
	{
		vTangents3D.resize(numUVVertices);
		vBitangents3D.resize(numUVVertices);
		...
		for (std::size_t i = 0; i < triIndicesUV.size(); ++i)
		{
			...
			RealT* cornerTangent = (RealT*)&vTangents3D[uvIdx];
			RealT* cornerBitangent = (RealT*)&vBitangents3D[uvIdx];

			addVec3((const RealT*)&cTangents3D[i], cornerTangent, cornerTangent);
			addVec3((const RealT*)&cBitangents3D[i], cornerBitangent, cornerBitangent);
		}
		...
		{
			normalizeVec3((RealT*)&vTangents3D[i]);
			normalizeVec3((RealT*)&vBitangents3D[i]);
		}
	}

	//-------------------------------------------------------------------------

	void orthogonalizeTSpace(const std::vector<Vec3>& normals3D,
		std::vector<Vec3>& tangents3D,
		std::vector<Vec3>& bitangents3D)
	{
		const std::size_t numVertices = normals3D.size();

		RealT correction[3];
		for (VIndexT i = 0; i < numVertices; ++i)
		{
			const RealT* nV = (const RealT*)&normals3D[i];

			RealT* bV = (RealT*)&bitangents3D[i];
			RealT* tV = (RealT*)&tangents3D[i];
